...
- `src/main/kotlin/com/tripleh/devtoolbox/`
  ...
  - `model/` — path/parameter model used for popup display.
    - `tools/` — the Dev Toolbox feature:
      ...
      - `diff/` — `TextDiffPanel` embeds the IDE's own diff viewer (`DiffManager.createRequestPanel` + editable `SimpleDiffRequest`): line numbers, per-line/per-word highlighting, gutter markers and change navigation come from the platform. Both panes editable, re-diffs live; once both sides parse via `JsonFormat` the editors are rebuilt once with the JSON file type (sticky upgrade, no downgrade while typing). No custom diff algorithm — don't reintroduce one.
